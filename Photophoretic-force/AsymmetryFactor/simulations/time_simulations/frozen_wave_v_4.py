...
from AsymmetryFactor.j1_with_gn_parallel import j1_with_gn_parallel
...
################################################################################## 
#   VERSÃO 4                                                                     #
#   FUNÇÃO DE GN PARALELIZADO                                                    #
#                                                                                #
#   -> inside simulations folder run $python time_simulations/frozen_wave_v_4.py #
##################################################################################
...
path = "./outputs/time_result/FW_v4/"
...
def j1_frozen_wave_with_varing_z0_values_l2_l4_G2_FW():    
    ...
    with open(time_file_name, mode="w", newline='') as time_file:
        ...
        with tqdm(colour=yellow, total=max_executions, desc="Calculating points", leave=True) as pbar:
            for e in range(max_executions):  
                
                start = timer()
                
                for i_x in x:
                    results_l2.append(j1_with_gn_parallel(ParticleAttributes(i_x, m_038, ur), fw_l2))
                    results_l4.append(j1_with_gn_parallel(ParticleAttributes(i_x, m_038, ur), fw_l4)*250)
