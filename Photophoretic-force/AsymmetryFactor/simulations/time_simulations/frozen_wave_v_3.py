...
from AsymmetryFactor.j1_parallel import j1_with_summation_parallel
...
path = "./outputs/time_result/FW_v3/"
...
def j1_frozen_wave_with_varing_z0_values_l2_l4_G2_FW():    
    ...
    with open(time_file_name, mode="w", newline='') as time_file:
        ...
        with tqdm(colour=green, total=max_executions, desc="Calculating points", leave=True) as pbar:
            for e in range(max_executions):  
                
                start = timer()
                
                for i_x in x:
                    results_l2.append(j1_with_summation_parallel(ParticleAttributes(i_x, m_038, ur), fw_l2, qnt_process))
                    results_l4.append(j1_with_summation_parallel(ParticleAttributes(i_x, m_038, ur), fw_l4, qnt_process)*250)
    ...
    # plot_graphic(results, x, x_label, y_label, title="FW_2_v3" ,legend=legend)
    # plot_graphic(results, x, x_label, y_label, title="FW_2_v3" ,legend=legend)
