...
def summation(particle, beam, change):
    """
    ...
    for i in range(1, (n_max+1)):
        ...
        if isinstance(beam, GaussAttributes):
            ...
            if change == 0: # Aq = 1
                gn = gn_frozen_wave_beam_with_Aq_1(i, beam.n, beam.k, beam.z0, beam.l, beam.q)
                gn1 = gn_frozen_wave_beam_with_Aq_1((i+1), beam.n, beam.k, beam.z0, beam.l, beam.q)
            
            elif change == 1: #Aq = defined integral
                gn = gn_frozen_wave_beam_with_define_integrate_in_Aq(i, beam.n, beam.k, beam.z0, beam.l, beam.q)
                gn1 = gn_frozen_wave_beam_with_define_integrate_in_Aq((i+1), beam.n, beam.k, beam.z0, beam.l, beam.q)   
...
def j1(particle, beam, change):    
    ...
    if isinstance(particle, ParticleAttributes) and isinstance(beam, BeamAttributes):
        ...
        summation_result = summation(particle, beam, change)
    else:
